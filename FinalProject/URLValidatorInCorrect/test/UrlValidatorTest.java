...
public class UrlValidatorTest extends TestCase {
   ...
   /**
    * Tests the URLValidator with different option settings and random URLs
    */
    public void testIsValidRandom() {
        ...
        setUp();
        testIsValidRandom(testUrlParts, UrlValidator.ALLOW_ALL_SCHEMES);
        // Allow all URL schemes and 2 slashes. Do not allow fragments
        setUp();
        testIsValidRandom(testUrlPartsOptions, options);
    }
   ...
   /**
    * Tests randomly generated URLs according to the URL partions and URLValidator options
    *
    * @param testObjects - An array containing elements that can be used for the different parts of a URL
    * @param options - The configuration options for the URLValidator
    */
    public void testIsValidRandom(Object[] testObjects, long options) {
        UrlValidator urlVal = new UrlValidator(null, null, options);

        for (int i = 0; i < 2000; i++) {
            // Randomize the URL sections
            randomizeTestPartsUrlIndex(testObjects);
            boolean expected = true;
            // Build the URL
            StringBuilder buffer = new StringBuilder();
            for (int testPartsIndexIndex = 0; testPartsIndexIndex < testPartsIndex.length; testPartsIndexIndex++) {
                int index = testPartsIndex[testPartsIndexIndex];
                ResultPair[] parts = (ResultPair[]) testObjects[testPartsIndexIndex];

                buffer.append(parts[index].item);
                expected &= parts[index].valid;
            }
   ...
   /**
    * Randomize the indices in the testPartsIndex member array.
    * The indices are bounded by the number of data available for
    * each URL partition.
    *
    * @param urlParts - An array containing elements that can be used for the different parts of a URL
    */
    private void randomizeTestPartsUrlIndex(Object[] urlParts) {
        for (int i = 0; i < urlParts.length; i++) {
            ResultPair[] pairs = (ResultPair[]) urlParts[i];
            int randomIndex = (int) (Math.random() * pairs.length);

            testPartsIndex[i] = randomIndex;
        }
    }
